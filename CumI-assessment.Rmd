...
cumi_buffer_intensity <- function(pressureData,pressureBufferModel) {
	...
	if (footprint & st_geometry_type(pressureData,by_geometry=FALSE) != "POINT") {
	   zone_footprint <- st_buffer(pressureData,0) %>% st_union() %>% st_sf() %>% st_cast("POLYGON") %>% mutate(intensity="loss") %>% rename_geometry("geometry")
	   if (exists("output")) {
...
cumi_process_pressure <- function(pressureData,pressureType,suffix) {
   ...
   # clip at biotope boundary and combine with partial impact layer
   #
   ...
   #
   intensity_layer <- cumi_buffer_intensity(pressureData,pressureType) %>% st_intersection(clipLayer)
   column <- paste("impact_",suffix,sep="")
   result <- st_cast(impact_map,"MULTIPOLYGON") %>% 
      my_union(intensity_layer) %>% 
      dplyr::mutate( {{ column }} := unname(mapply(cumi_impact,intensity,sens))) %>% 
      dplyr::select(-int)
   return(result)
}
```
...
biotopeLayer <- st_read(biotope_file) %>% st_transform(coordSystem) %>% st_cast("MULTIPOLYGON") %>% dplyr::select(-s_subsurf) %>% st_make_valid()
```
...
The fishery data were provided by ICES and then put into HELCOM Maps and Data Service. This version of the script uses a preprocessed version of these raw data, where all years have been combined into one GIS layer and the pressure has been smoothed from c-squares into an 1x1 km grid. *The next version of the script will also contain the necessary steps to make the preprocessing.*
...
# we here provide a pre-calculated file, based on the first part of the script up to here.
...
#
# You might even want to use ArcGIS for this particular 'union' task below as it is even significantly faster than QGIS.
#
cumi_message("processing of bottom trawling fishery ...")
if (get_pre_calculated_data) {
	...
	fishing_x <- fishing_x %>% dplyr::select(-c(ET_ID,H_sub_code,H_sub_name,location,SURFSAR_11,SURFSAR_12,SURFSAR_13,SURFSAR_14,SURFSAR_15,SURFSAR_16,sar_avg,int))
	
	# the my_union() in the following line is what takes 99.9% of the time in this pressure:
	impact_map %>% st_cast("MULTIPOLYGON") %>% my_union(fishing_x) %>% dplyr::mutate(impact_6 = unname(mapply(cumi_impact,MOP,s_surf))) %>% dplyr::select(-MOP)
}
...
impact_map <- cumi_process_pressure(mari_1,"mariculture","7A")
...
# In this case, MOP is the same as intensity as frequency is not applicable:
impact_map <- cumi_process_pressure(mari_2,"mariculture","7B")
...
# In this case, MOP is the same as intensity as frequency is not applicable:
impact_map <- cumi_process_pressure(mari_3,"mariculture","7C")

```

### 9. Extraction and disposal of sediments

```{r}
cumi_message("processing of Extraction and disposal of sediments ...")
```
...
impact_map <- cumi_process_pressure(extraction_x,"extraction","9A")
...
# In this case, MOP is the same as intensity as frequency is currently ignored:
impact_map <- cumi_process_pressure(deposit_1,"depositing","9B")
...
# In this case, MOP is the same as intensity as frequency is currently ignored:
impact_map <- cumi_process_pressure(deposit_2,"depositing","9C")
...
# In this case, MOP is the same as intensity as frequency is currently ignored:
impact_map <- cumi_process_pressure(dredge_1,"dredging CAP","9D")
...
# In this case, MOP is the same as intensity as frequency is currently ignored:
impact_map <- cumi_process_pressure(dredge_2,"dredging M","9E")
...
# In this case, MOP is the same as intensity as frequency is currently ignored:
# above 5000m3:
impact_map <- cumi_process_pressure(filter(dredge_3,volume>5000),"dredging CAP2","9F")
# below or equal to 5000m3:
impact_map <- cumi_process_pressure(filter(dredge_3,volume<=5000 | is.na(Amount_ton)),"dredging CAP1","9G")
...
# In this case, MOP is the same as intensity as frequency is currently ignored:
impact_map <- cumi_process_pressure(dredge_4,"dredging M","9H")
...
impact_map <- cumi_process_pressure(pipeline_o,"pipeline O","3o")
...
impact_map <- cumi_process_pressure(cable_c,"cable C","1c")
...
# In this case, MOP is the same as intensity as frequency does not apply:
impact_map <- cumi_process_pressure(cable_o,"cable O","1o")
```
...
# In this case, MOP is the same as intensity as frequency does not apply:
impact_map <- cumi_process_pressure(oil_x,"platform O","4A")
```
...
impact_map <- cumi_process_pressure(coastalDef_c,"coastalDef C","2c")
...
# In this case, MOP is the same as intensity as frequency does not apply
impact_map <- cumi_process_pressure(coastalDef_o,"coastalDef O","2o")
```


#### C. harbours 200m
•	Polygondaten mit 200 m gepuffert (Ausmaß 400 m Verlust)



### Shipping
### 14. shipping
Data layer from HELCOM:
- shipping
- fuel consumption

```{r eval=FALSE}
shipping_file <- "..."
shipping_x <- st_read(shipping_file) %>% ...
## TODO: build a cumi_shipping_intensity() function after the template of cumi_buffer_intensity()
int_layer <- cumi_shipping_intensity(my_sf,type) %>% st_intersection(clipLayer)
