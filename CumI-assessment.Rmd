...
if (get_pre_calculated_data) {
	...
	impact_map %>% st_cast("MULTIPOLYGON") %>% my_union(fishing_x) %>% dplyr::mutate(impact_1 = unname(mapply(cumi_impact,MOP,s_surf))) %>% dplyr::select(-MOP)
}
...
# we only gave the clipping left and can then directly combine with the biotope layer:
shipping_x <- shipping_x %>% st_intersection(clipLayer)
impact_map <- st_cast(impact_map,"MULTIPOLYGON") %>% 
   my_union(shipping_x) %>% 
   dplyr::mutate(impact_10 = unname(mapply(cumi_impact,MOP,sens))) %>% 
   dplyr::select(-c(density,depth,depthzone,intensity,MOP)) %>%
...
# initialize a new column 'c_impact' for the cumulative impact
initialColumn <- impactColumns[1]
impact_map <- impact_map %>% mutate(c_impact = .data[[initialColumn]])

# and set the value to the value of the first impact in the column list

# prepare an easy access lookup for the impact cumulation:

# loop over each polygon/multipolygon
	# foreach pair, do the cumulation
```
...
Also, some visualizations will be implemented on the resulting impact map, such as graphs on the aerial extent of the various impact levels over the whole assessment area or certain subdivisions of that area.

Further, the pre-processing of the bottom trawling data and the shipping data will be implemented.
