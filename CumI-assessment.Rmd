...
if (do_cumi) {
	st_write(impact_map_disturbance,file.path(myDir,paste("Results/CumI-assessment-DE-",format(Sys.time(),"%Y-%m-%dT%H%M"),".gpkg",sep= "")))
	st_write(impact_map_loss,file.path(myDir,paste("Results/physical_functional-loss-DE-",format(Sys.time(),"%Y-%m-%dT%H%M"),".gpkg",sep= "")))
