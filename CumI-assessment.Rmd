...
cumi_buffer_intensity <- function(pressureData,pressureBufferModel) {
	...
	if (footprint & (st_geometry_type(pressureData,by_geometry=FALSE) == "POLYGON" |
	    st_geometry_type(pressureData,by_geometry=FALSE) == "MULTIPOLYGON")) {
...
```{r}
if (! use_pre_calculated_data && do_cumi) {
	cumi_message("starting with German biotopes ...")
	biotopeBaltic_file <- "/Users/Torsten/Marilim/GIS/MSRL/Benthische Lebensräume/BHT-2023.gpkg"
	areaCountryBiotopeLayer <- st_read(biotopeBaltic_file, layer="BHT") %>%
		st_make_valid() %>%
...
if (! use_pre_calculated_data && do_cumi) {
	...
	for(i in 1:nrow(tmp)) {
		...
		if (sensi %in% c("Chara","Fucus","Furcellaria","Mytilus","Zostera")) {
			...
			if (bio == "Na" | is.na(sal)) {
				...
				# if the biotope is unclear,
				# precautionarily assume a higher sensitivity:
				tmp$sens[i] <- "high"
				tmp$s_surf[i] <- "high"
			} else {
				# standard case:
	...
	st_write(biotopeLayer,file.path(myDir,"Biotopes/biotopes-pre_calculated-DE.gpkg"))
}
```
...
if (use_pre_calculated_data && do_cumi) {
	...
	biotope_file <- file.path(myDir,"Biotopes/biotopes-pre_calculated-DE.gpkg")
...
```{r}
if (store_pressure_maps) {
	pressure_maps_file <- file.path(myDir,paste("Results/CumI-pressure-maps-DE-",format(Sys.time(),"%Y-%m-%dT%H%M"),".gpkg",sep= ""))
}
```
...
if (use_pre_calculated_data) {
	...
	if (do_cumi) {
		...
		st_write(impact_map,file.path(myDir,"Fishery/fishery_pre_calculated-DE.gpkg"))
	}
}
...
if (do_cumi) {
	...
	impact_map$confidence_3A <- "d1t3s1"
}
...
if (do_cumi) {
	st_write(impact_map_disturbance,file.path(myDir,paste("Results/CumI-assessment-EUSeaMap-DE-",format(Sys.time(),"%Y-%m-%dT%H%M"),".gpkg",sep= "")))
	st_write(impact_map_loss,file.path(myDir,paste("Results/physical_functional-loss-EUSeaMap-DE-",format(Sys.time(),"%Y-%m-%dT%H%M"),".gpkg",sep= "")))
	...
	# st_write(impact_map,file.path(myDir,paste("Results/CumI-assessment-total-DE",format(Sys.time(),"%Y-%m-%dT%H%M"),".gpkg",sep= "")))
}
```
