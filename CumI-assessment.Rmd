...
# treat SAR vlues lower than 0.05 as zero:
foreach (myColumn in c('2011',...,'2016')) {
   fishing_x <- fishing_x %>% ...ifelse( myVar >=0.05,1,0)
}

# hier SAR in MOP umwandeln
fishing_x <- fishing_x %>%  mutate(sar_avg = sum(...)/6) %>% mutate(fishing_x,int= case_when(
   sar_avg < 0.33 ~ 'very low',
   sar_avg < 0.66 ~ 'low',
   sar_avg < 2 ~ 'moderate',
   TRUE < 'high'
))

impact_map %>% st_cast("MULTIPOLYGON") %>% my_union(fishing_x) %>% dplyr::mutate(impact_6 = unname(mapply(cumi_impact,int,sens))) %>% dplyr::select(-int)
...
•	Finfish mariculture: 
o	Punktdaten mit 150 m gepuffert (Ausmaß 300 m Verlust)
o	Punktdaten mit Puffermodell gepuffert (puffer-v3-mariculturelin.modell3, beginnend ab 150 m Verlustfläche) 1000m, Differenz zwischen Pufferergebnis und Verlustfläche für die Zuordnung Störung (Ausmaß 2 km mit linearer Abnahme)
...
o	Polygone direkt als Verlustflächen verwendet, Punktdaten mit 150 m gepuffert (Ausmaß 300 m Verlust)
o	Punktdaten mit Puffermodell gepuffert (puffer-v3-mariculturelin.modell3, beginnend ab 150 m Verlustfläche) 1000m, Differenz zwischen Pufferergebnis und Verlustfläche für die Zuordnung Störung (Ausmaß 2 km mit linearer Abnahme)
...
•	Puffermodell ‚ puffer-v3-deposit.modell3‘ 500 m, Zuordnung Störung
•	Die Bearbeitung der Daten erfolgte sowohl für Punktdaten als auch für Polygone (deposit points/deposit areas)
