...
myDir <- "/Users/Torsten/Marilim/BLANO-MSRL/GIS/CumI"
myHELCOMDir <- "/Users/Torsten/Library/CloudStorage/OneDrive-MariLimGesellschaftfürGewässeruntersuchungmbH/MSRL/CumI-R/Data-2023-01-20"
myGeneralDir <- "/Users/Torsten/Marilim/GIS"
```

***NOTE:*** This documentation was done for the whole Baltic dataset. Thus, the confidecne rating and most of the text description do not apply to the national German MSFD assessment that is being done in this version !!!
...
my_union <- function(a,b) {
	...
	if (st_geometry_type(result,by_geometry=FALSE) == "GEOMETRY") {
		...
		result <- result %>% filter(st_geometry_type(result) %in% c("POLYGON","MULTIPOLYGON"))
	}
	return(result)
}
```
...
```{r cumi_impact_matrix}
cumi_impact_matrix <- tribble(
	~sensitivity, ~MOP_loss, ~MOP_high, ~MOP_moderate, ~MOP_low,    ~MOP_verylow,
	"high",       "loss",    "high",    "high",        "m2",        "m1",
	"moderate",   "loss",    "high",    "m3",          "m1",        "low",
	"low",        "loss",    "m2",      "m1",          "low",       "very low",
	"very low",   "loss",    "m1",      "low",         "very low",  "very low"
)
...
cumi_buffers <- tribble(
	...
	"groyne",		"groyne HyMo buffer",          0.5,				  5,			    5,				    5,			    5,					 1
)
...
cumi_buffer_intensity <- function(pressureData,pressureBufferModel) {
	...
	if (footprint & (st_geometry_type(pressureData,by_geometry=FALSE) %in% c("POLYGON","MULTIPOLYGON"))) {
...
cumi_process_pressure_no_buffering <- function(pressureData,pressureType,suffix,MOP) {
	#
	# do NOT apply a buffer model to a GIS layer with pressures,
	# but combine with partial impact layer directly
	# applying 'high' impact
	#
	# *** ONLY USED FOR GERMAN DATA DIRECLTY DELIVERED and PREPROCESSED By HyMo GRPUP ***
	#
	# pressureData - sf layer with pressure data
	# pressureType - layer type from tribble `cumi_buffers`, column ~pressure
	# suffix - suffix to append to the string "impact_" as name for the column to be filled with the calculated impacts
	# MOP = pressure intensity to apply (or "loss")
	#
	# This function for now treats intensity as MOP, as we do not have frequceny data  yet
	#
	# The function uses the global `impact_map` sf layer to add the pressure
	...
	#
	#
	intensity_layer <- pressureData %>% mutate(intensity=MOP) %>% rename_geometry("geometry")
	if (store_pressure_maps) {
		st_write(dplyr::rename(intensity_layer,MOP = intensity), pressure_maps_file, layer=paste("mop_",suffix,sep=""))
	}
	if (do_cumi) {
...
Sometimes, it can be necessary to skip the step where the sensitivities are modified by the layer with the distribution of some specific species. This can be configured here:

```{r cumi_skip_species}
cumi_skip_species <- FALSE
```

The files produces by this script have specific names. You can add a suffix to these names when you want to distinguish several scenarios. The suffix will be inserted into the file just before the date/time portion if there is one, otherwise it will be inserted at the end:

```{r cumi_file_suffix}
cumi_file_suffix <- "DE"
#cumi_file_suffix <- "EUSeaMap-DE"
#cumi_file_suffix <- "HOLAS3"
```
...
## The biotope map
The biotope map is based on the *German BHT map*, dated March 2023.
...
```{r biotopeMap eval=FALSE}
if (! use_pre_calculated_data && do_cumi) {
	cumi_message("starting with biotopes ...")
...
```{r biotopeMapDE}
areaCountryBiotopeLayer <- st_read(file.path(myGeneralDir,"MSRL/Benthische Lebensräume/MSFD_BHT_OHT_HUB/BHT-OHT-2023-07-18.gpkg"),"MSRL_BHT_OHT_160723") %>%
	filter(GEBIET == "Ostsee") %>%
	select(-c(ADMIN,GEBIET,MARIN,SUBREGION,HELCOM_ID,MRU,OHT_en,BHT_EN,namespace,nameTxtInt,desigBegin,DataBasis,Sources)) %>%
	mutate(Territory = 'Germany')
...
if (! use_pre_calculated_data && do_cumi) {
  ...
  species_folder <- file.path(myHELCOMDir,"Biotopes/Benthic species HOLAS 3")
  ...
  if (! cumi_skip_species) {
    for(myfile in c(
      "_ags_EC_23/EC_23.tif",
      "_ags_EC_25/EC_25.tif",
      "_ags_EC_21/EC_21.tif",
      "_ags_EC_24/EC_24.tif",
      "_ags_EC_22/EC_22.tif"
    )) {
      myShort <- substring(myfile,9,10)
      myName <- case_when(
        myShort == "23" ~ "Chara",
        myShort == "25" ~ "Fucus",
        myShort == "21" ~ "Furcellaria",
        myShort == "24" ~ "Mytilus",
        myShort == "22" ~ "Zostera",
        TRUE ~ "undefined"
      )
      cumi_message(paste("... ", myfile, ", ",myName, sep=""))
      ...
      if (myName == "Zostera") {
        depths <- rast(file.path(myHELCOMDir,"Base/depth/w001001.adf"))
...
if (! use_pre_calculated_data && do_cumi) {
	...
	salinity_file <- file.path(myHELCOMDir,"../Data-2022/Salinity/BALANCE_BOTTOM_SALINITY.tif")
	...
	# (this following can be done here in the DE case and takes roughly an hour or three, but doing in in ArcGIS is quicker with roughly 20 minutes:)
...
if (! use_pre_calculated_data && do_cumi) {
	...
	cumi_Sensitivities <- tribble(
		...
		"Germany", "circa mix", "5-7.5",  "high",     "high",
		"Germany", "circa mix", "7.5-11", "high",     "high",
		"Germany", "circa mix", "11-18",  "high",     "high",
		"Germany", "circa mix", "18-30",  "high",     "high",
		...
		"Germany", "infra mix", "5-7.5",  "high",     "high",
		"Germany", "infra mix", "7.5-11", "high",     "high",
		"Germany", "infra mix", "11-18",  "high",     "high",
		"Germany", "infra mix", "18-30",  "high",     "high",

		"Germany", "infra mud", "< 5",    "moderate",     "moderate", # analogue to 5-7.5 psu in this group
		...
		"Germany", "infra sand", "< 5",    "moderate",    "moderate", # analogue to 5-7.5 psu in this group
	...
	for(i in 1:nrow(tmp)) {
		...
		shortName <- case_when(
			...
			bio == "Grobsedimente des Circalitorals" ~ "circa hard",
			bio == "Grobsediment des Circalitorals" ~ "circa hard",
			bio == "Mischsedimente des Circalitorals" ~ "circa mix",
			bio == "Mischsediment (Hartboden) des Circalitorals" ~ "circa mix",
			bio == "Schlickböden des Circalitorals" ~ "circa mud",
			bio == "Sandböden des Circalitorals" ~ "circa sand",

			bio == "Infralittoral rock and biogenic reef" ~ "infra hard",
			bio == "Grobsedimente des Infralitorals" ~ "infra hard",
			bio == "Grobsediment des Infralitorals" ~ "infra hard",
			bio == "Mischsedimente des Infralitorals" ~ "infra mix",
			bio == "Mischsediment (Hartboden) des Infralitorals" ~ "infra mix",
			bio == "Schlickböden des Infralitorals" ~ "infra mud",
			bio == "Sandböden des Infralitorals" ~ "infra sand",
			bio == "Felsen und biogene Riffe des Infralitorals" ~ "infra hard",
		...
		if (sensi %in% c("Chara","Fucus","Furcellaria","Mytilus","Zostera")) {
			...
			if (bio == "Na" | bio == "NA" | is.na(sal)) {
				...
				# (this applies to the EUSeapMap which sometimes assigns two habitats at the same time):
	...
	st_write(biotopeLayer,file.path(myDir,paste("Biotopes/biotopes-pre_calculated-",cumi_file_suffix,".gpkg",sep="")))
}
```
...
if (use_pre_calculated_data && do_cumi) {
	...
	biotope_file <- file.path(myDir,paste("Biotopes/biotopes-pre_calculated-",cumi_file_suffix,".gpkg",sep=""))
...
```{r prep3}
if (store_pressure_maps) {
	pressure_maps_file <- file.path(myDir,paste("Results/CumI-pressure-maps-",cumi_file_suffix,"-",format(Sys.time(),"%Y-%m-%dT%H%M"),".gpkg",sep= ""))
}
```
...
if (use_pre_calculated_data) {
	...
	fishing_file_pre <- file.path(myDir,paste("Fishery/fishery_pre_calculated-",cumi_file_suffix,".gpkg",sep=""))
	...
	fishing_file <- file.path(myHELCOMDir,"Fishery/fishery_surface.shp")
	if (do_cumi) {
	  # the commented lines should only be used when the pressure layer is much larger
	  # than the biotope layer. Otherwise, this step takes just too long and the polygons
	  # outside the biotope boundary are removed in a later step anyway:
	...
	for(myYear in assessmentYears) {
	  ...
	  ## the following doesn't work ... ToDo: fix this!
	...
	if (do_cumi) {
		...
		st_write(impact_map,file.path(myDir,paste("Fishery/fishery_pre_calculated-",cumi_file_suffix,".gpkg",sep="")))
	}
}
...
mari_1_file <- file.path(myHELCOMDir,"Finfish mariculture/Finfish_PLC.shp")
...
mari_2_file <- file.path(myHELCOMDir,"Shellfish mariculture/Aquaculture_Shellfish_EMODnets.shp")
...
extraction_file <- file.path(myHELCOMDir,"Extraction of sand and gravel/Extraction.shp")
...
# the DE data are nealry identical to HELCOM dataset `file.path(myHELCOMDir,"depositing of dredged material/depositing_polygons.shp")`
# however, the DE data are buffered already and denote disturbance in the 'high' category
deposit_1_file <- file.path(myDir,"../HyMo/Ostsee_LAEA_AbgabeD6/PhysicalDisturbance_UmlagerungUnterhaltung_LAEA.shp")
deposit_1 <- st_read(deposit_1_file)

if (do_cumi || store_pressure_maps) {
	# In this case, MOP is the same as intensity as frequency is currently ignored:
	impact_map <- cumi_process_pressure_no_buffering(deposit_1,"depositing","3B","high")
}
if (do_cumi) {
	# confidence:
...
```{r eval=FALSE}
### mostly overlaps with DE-HyMo data, this skipping:
...
```{r eval=FALSE}
### only cables to OWF, skipping here as they are not contained in DE HyMo dataset:
...
```{r eval=FALSE}
### seems to be covered by DE HayMo dataset, skipping
...
```{r}
dredge_p_file <- file.path(myHELCOMDir,"dredging/Dredging_points.shp")
...
```{r eval=FALSE}
### skipping here, as Germany has the HyMo dataset
...
```{r}
cumi_message("... pipelines (areas) ...")
pipeline_file1 <- file.path(myDir,"../HyMo/Ostsee_LAEA_AbgabeD6/Nordstream_1_2_LAEA_Deutschland.shp")
...
if (do_cumi || store_pressure_maps) {
	...
	impact_map <- cumi_process_pressure_no_buffering(pipeline_c,"pipeline C","4A","loss")
}
...
```{r eval=FALSE}
### no additional data in the HyMo DE dataset
...
cable_file <- file.path(myDir,"../HyMo/Ostsee_LAEA_AbgabeD6/Kabel_2016_2021_LAEA.shp")
...
cumi_message("... cables ...")
if (do_cumi || store_pressure_maps) {
	impact_map <- cumi_process_pressure_no_buffering(filter(cable_oc),"cable O","4C","high")
}
if (do_cumi) {
	# confidence:
...
cable_file2 <- file.path(myDir,"../HyMo/Ostsee_LAEA_AbgabeD6/PiV_Ostsee.shp")
cable_oc2 <- st_read(cable_file2)

cumi_message("... cables 2 ...")
if (do_cumi || store_pressure_maps) {
	impact_map <- cumi_process_pressure_no_buffering(filter(cable_oc),"cable O","4D","high")
}
if (do_cumi) {
	# confidence:
...
```{r}
cumi_message("... wind farms and platforms ...")
wind_file <- file.path(myDir,"../HyMo/Ostsee_LAEA_AbgabeD6/PlattformenUndWindkraftanlagen_LAEA.shp")
wind_x <- st_read(wind_file)

if (do_cumi || store_pressure_maps) {
	# In this case, MOP is the same as intensity as frequency does not apply:
	impact_map <- cumi_process_pressure_no_buffering(wind_x,"OWF O","5A","loss")
}
if (do_cumi) {
	impact_map <- impact_map %>% mutate(confidence_5A = "d1t0s1")
}

cumi_message("... marine constructions - physical loss ...")
constr1_file <- file.path(myDir,"../HyMo/Ostsee_LAEA_AbgabeD6/PhysicalLossBauwerke_Ostsee_LAEA.shp")
constr1 <- st_read(constr1_file)

if (do_cumi || store_pressure_maps) {
	# In this case, MOP is the same as intensity as frequency does not apply:
	impact_map <- cumi_process_pressure_no_buffering(constr1,"OWF O mono","5B","loss")
}
if (do_cumi) {
	impact_map <- impact_map %>% mutate(confidence_5B = "d1t0s1")
}

cumi_message("... marine constructions - physical disturbance ...")
constr2_file <- file.path(myDir,"../HyMo/Ostsee_LAEA_AbgabeD6/PhysicalDisturbanceBauwerke_Ostsee_LAEA.shp")
constr2 <- st_read(constr2_file)

if (do_cumi || store_pressure_maps) {
	# In this case, MOP is the same as intensity as frequency does not apply:
	impact_map <- cumi_process_pressure_no_buffering(constr2,"OWF O mono","5C","high")
}
if (do_cumi) {
	impact_map <- impact_map %>% mutate(confidence_5C = "d1t0s1")
}
...
```{r eval=FALSE}
## no HELCOM data in DE area
# main polygon data, all operational:
def_1_file <- file.path(myHELCOMDir,"Coastal defence/CoastalDefense_polygons.shp")
...
# line data:
def_2_file <- file.path(myHELCOMDir, "Coastal defence/CoastalDefence_lines_APPEND.shp")
def_2 <- st_read(def_2_file)

if (do_cumi || store_pressure_maps) {
	# MOP is the same as intensity as frequency does not apply:
	impact_map <- cumi_process_pressure(filter(def_2,Status == "under construction" & substr(Type,1,6) != "groyne"),"coastalDef C","6B")
}
if (do_cumi) {
...
if (do_cumi || store_pressure_maps) {
	# MOP is the same as intensity as frequency does not apply:
	impact_map <- cumi_process_pressure(filter(def_2,Status == "operational" & substr(Type,1,6) != "groyne"),"coastalDef O","6C")
}
if (do_cumi) {
	impact_map <- impact_map %>% mutate(confidence_6C = "d1t0s1")
}

# groynes treated specially as there is a HyMo buffer model for this:
if (do_cumi || store_pressure_maps) {
	# MOP is the same as intensity as frequency does not apply:
	impact_map <- cumi_process_pressure(filter(def_2, substr(Type,1,6) == "groyne"),"groyne","6D")
}
if (do_cumi) {
...
ship_folder <- file.path(myHELCOMDir,"shipping")
...
depths <- rast(file.path(myHELCOMDir,"Base/depth rasters/depth_1x1km.tif"))
...
if (do_cumi) {
	# the union in the following statement took roughly 30 hours on my machine using the QGIS function,
  # for the DE case it runs approx. 9-10 hours,
  # on ArcGIS the union fails with an unspecific error:
...
if (do_cumi) {
	st_write(impact_map_disturbance,file.path(myDir,paste("Results/CumI-assessment-",cumi_file_suffix,"-",format(Sys.time(),"%Y-%m-%dT%H%M"),".gpkg",sep="")))
	st_write(impact_map_loss,file.path(myDir,paste("Results/physical_functional-loss-",cumi_file_suffix,"-",format(Sys.time(),"%Y-%m-%dT%H%M"),".gpkg",sep="")))
...
st_write(impact_map,file.path(myDir,paste("Results/CumI-assessment-total-",cumi_file_suffix,"-",format(Sys.time(),"%Y-%m-%dT%H%M"),".gpkg",sep= "")))
}
```
