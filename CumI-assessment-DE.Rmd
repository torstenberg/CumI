...
```{r biotopeMapDE}
areaCountryBiotopeLayer <- st_read(file.path(myGeneralDir,"MSRL/Benthische Lebensräume/BHT-OHT-2023.gpkg")) %>%
	filter(GEBIET == "Ostsee") %>%
	rename(Biotope = BHT) %>%
	select(-c(ADMIN,OBJECTID,ID_ORIGIN,NAME,GEBIET,MARIN,SUBREGION,HELCOM_ID,MRU,WRRL_WK,BHT_EN,OHT,OHT_EN,LRT,LRT_EN,BHT_EN_1,localId,namespace,versionId,thematicId,nameTxtInt,nameText_1,nameTxtLan,desigBegin,desigEnd,ImReportVe,Area_ha,AREA_qkm,Shape_Length,Shape_Area)) %>%
